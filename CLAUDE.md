...
## Стек
- **Backend**: Django 4.2 + SQLite + gunicorn + nginx, сервер на Digital Ocean (переезд на Яндекс в процессе)
...
Favorite:  user (FK → User), product (FK → Product), created_at
           — unique_together (user, product); ordering = ['-created_at']
```
...
**Размеры:** 100, 200, 300, 600 px (квадратные, `thumbnail()` с LANCZOS, JPEG 85%, EXIF-ориентация исправляется через `ImageOps.exif_transpose`)
...
### Favorites API
- `GET /api/favorites/` — список избранного (требует токен); поля: `product_id`, `product_name`, `cover_url`, `cover_url_100`, `created_at`
- `POST /api/favorites/` — добавить `{"product_id": N}`; 201 при создании, 200 если уже есть; скрытые товары → 404
- `DELETE /api/favorites/<product_id>/` — удалить; 204 в любом случае (идемпотентно)
...
- **CategoryScreen**: плитка 2 колонки, круглые обложки с мягкими краями (radial gradient), название + "товаров: N" в полупрозрачном прямоугольнике поверх нижней четверти круга. TopAppBar содержит кнопку "Меню" (dropdown): для staff — Чаты (с красной точкой), Комментарии (с красной точкой), Избранное, Поиск, Аккаунт; для обычного пользователя — Чат (с красной точкой), Комментарии (свои), Избранное, Поиск, Аккаунт; для незалогиненных — Войти. В самом низу меню — версия приложения (`BuildConfig.VERSION_NAME`, `labelSmall`). Красная точка на "Меню" если хоть что-то непрочитано. Polling счётчиков каждые 15 сек (пока CategoryScreen активен). При смене токена загружает `DataCache.favoriteProductIds`.
- **ProductListScreen**: сетка 4 колонки, квадратные фото, оверлей "N фото" снизу слева
- **ProductDetailScreen**: название товара над галереей (жирно, мелко). TopAppBar показывает название категории. Галерея — сетка 3 колонки (квадратные тайлы, `ContentScale.Crop`), тап → fullscreen. Если переход из чата по `[product:id:page]` — fullscreen открывается автоматически на нужном фото. Кнопка "В чат" в fullscreen viewer (правый нижний угол, рядом со "Скачать" и "Ссылка"): для пользователя — диалог "Вас интересует данный продукт?" (Да/Нет, просто в чат), "Да" отправляет сообщение `"Интересует товар «name» (фото N)\n[product:id:page]"` и переходит в чат; для staff — диалог "В чат с этим товаром?" (Да/Нет), "Да" предзаполняет `[product:id:page]` в поле ввода выбранного чата. Для staff: имена авторов комментариев — кликабельные ссылки в чат. Кнопка "Назад" ведёт в категорию (не в чат).
- **ProductDetailScreen**: ... Иконка сердечка (Favorite/FavoriteBorder) под галереей — для незалогиненных открывает AuthDialog; оптимистичное обновление с откатом при сетевой ошибке.
- **FullscreenImageViewer**: пинч-зум (до 8×), панорамирование при zoom > 1, одиночный тап — закрыть. Свайп влево/вправо — листать фото (циклически, `Int.MAX_VALUE` виртуальных страниц). Свайп отключается при zoom > 1 чтобы не конфликтовать с панорамированием. Снизу: иконка сердечка "Избранное" (всегда видна), "В чат" (всегда — для незалогиненных открывает AuthDialog), "Скачать", "Ссылка".
- **FavoritesScreen**: список избранных товаров (72px обложка + название). Свайп влево → красный фон с иконкой корзины → диалог "Удалить из избранного?" (Удалить/Отмена). `SwipeToDismissBox` с `confirmValueChange = false` — свайп сбрасывается автоматически, диалог показывается через `pendingDeleteId`.
...
|---|---|---|
| `catalog/tests.py` | 16 классов | CategoryList, ProductList, ProductDetail, CommentList (включая `user_id`/`user_email`), MyComments, Search (изоляция user/staff), StaffComments CRUD, Mobile2 views, Presign, **FavoriteAPI** (list/add/duplicate/hidden/delete/auth/isolation/fields) |
